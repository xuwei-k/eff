...
trait safe {
  ...
  implicit class SafeEffectOps[R, A](e: Eff[R, A]) {
    ...
    def runSafe[U](using m: Member.Aux[Safe, R, U]): Eff[U, (Throwable Either A, List[Throwable])] =
      SafeEffect.runSafe[R, U, A](e)

    def execSafe[U](using m: Member.Aux[Safe, R, U]): Eff[U, Throwable Either A] =
    ...
    def attempt(using m: Safe /= R): Eff[R, Throwable Either A] =
