...
trait SafeInterpretation extends SafeCreation { outer =>
   ...
   */
  def runSafe[R, U, A](effect: Eff[R, A])(using m: Member.Aux[Safe, R, U]): Eff[U, (ThrowableEither[A], List[Throwable])] =
    runInterpreter[R, U, Safe, A, Out[A]](effect)(safeInterpreter[U, A])

  /** run a safe effect but drop the finalizer errors */
  def execSafe[R, U, A](r: Eff[R, A])(using m: Member.Aux[Safe, R, U]): Eff[U, ThrowableEither[A]] =
    runSafe(r).map(_._1)

  /**
   * Attempt to execute a safe action including finalizers
   */
  def attemptSafe[R, A](effect: Eff[R, A])(using m: Safe /= R): Eff[R, (ThrowableEither[A], List[Throwable])] =
   ...
   * Eff.bracketLast
   */
  def bracket[R, A, B, C](acquire: Eff[R, A])(use: A => Eff[R, B])(release: A => Eff[R, C])(using m: Safe /= R): Eff[R, B] =
    for {
      a <- acquire
   ...
   */
  def otherwise[R, A](action: Eff[R, A], onThrowable: Eff[R, A])(using m: Safe /= R): Eff[R, A] =
   ...
   */
  def catchThrowable[R, A, B](action: Eff[R, A], pureValue: A => B, onThrowable: Throwable => Eff[R, B])(using m: Safe /= R): Eff[R, B] =
   ...
   */
  def whenFailed[R, A](action: Eff[R, A], onThrowable: Throwable => Eff[R, A])(using m: Safe /= R): Eff[R, A] =
   ...
   * The final value type is the same as the original type
   */
  def whenThrowable[R, A](action: Eff[R, A], onThrowable: PartialFunction[Throwable, Eff[R, A]])(using m: Safe /= R): Eff[R, A] =
    recoverThrowable(action, identity[A], onThrowable)

  /**
   * try to execute an action an report any issue
   */
  def attempt[R, A](action: Eff[R, A])(using m: Safe /= R): Eff[R, Throwable Either A] =
    catchThrowable(action, Right[Throwable, A], (t: Throwable) => pure(Left(t)))

  /**
   * ignore one possible exception that could be thrown
   */
  def ignoreException[R, E <: Throwable: ClassTag, A](action: Eff[R, A])(using m: Safe /= R): Eff[R, Unit] =
   ...
   */
  def safeMemo[R, A](key: AnyRef, cache: Cache, e: Eff[R, A])(using safe: Safe /= R): Eff[R, A] =
