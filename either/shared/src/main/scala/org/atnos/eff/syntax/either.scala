...
trait either {
  given eitherExtension: AnyRef with {
    ...
    extension [R, A](e: Eff[R, A]) {
      ...
      def runEitherU[E, U](using m: Member.Aux[Either[E, *], R, U]): Eff[U, E Either A] =
        EitherInterpretation.runEither(e)(using m)

      def runEitherCombine[E, U](using m: Member.Aux[Either[E, *], R, U], s: Semigroup[E]): Eff[U, E Either A] =
        EitherInterpretation.runEitherCombine(e)(using m, s)

      def attemptEither[E](using m: Either[E, *] /= R): Eff[R, E Either A] =
