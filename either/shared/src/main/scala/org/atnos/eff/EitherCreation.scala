...
trait EitherCreation {
  ...
  def optionEither[R, E, A](option: Option[A], e: => E)(using member: Either[E, *] |= R): Eff[R, A] =
    option.fold[Eff[R, A]](left[R, E, A](e))(right[R, E, A])

  /** create an Either effect from a single Either value */
  def fromEither[R, E, A](Either: E Either A)(using member: Either[E, *] |= R): Eff[R, A] =
    send[Either[E, *], R, A](Either)

  /** create a failed value */
  def left[R, E, A](e: E)(using member: Either[E, *] |= R): Eff[R, A] =
    send[Either[E, *], R, A](Left(e))

  /** create a correct value */
  def right[R, E, A](a: A)(using member: Either[E, *] |= R): Eff[R, A] =
    send[Either[E, *], R, A](Right(a))

  /** create an Either effect from a value possibly throwing an exception */
  def fromCatchNonFatal[R, E, A](a: => A)(onThrowable: Throwable => E)(using member: Either[E, *] |= R): Eff[R, A] =
    fromEither(Either.catchNonFatal(a).leftMap(onThrowable))

  /** create an Either effect from a value possibly throwing a Throwable */
  def catchNonFatalThrowable[R, A](a: => A)(using member: ThrowableEither |= R): Eff[R, A] =
