...
class EffApplicativeSpec(implicit ee: ExecutionEnv)
...
"""
  ...
  def interleavedApplicative2 = {
    ...
    FutureEffect.futureAttempt(action).runOption.runAsync must beSome(
      Right(List(1, 2, 3, 4)): Either[Throwable, List[Int]]
    ).await
  }
