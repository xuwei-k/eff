...
final case class TimedFuture[A](
  callback: (Scheduler, ExecutionContext) => Future[A],
...
trait FutureCreation extends FutureTypes {
  ...
  final def fromFutureWithExecutors[R: _future, A](
    c: (Scheduler, ExecutionContext) => Future[A],
    timeout: Option[FiniteDuration] = None
  ): Eff[R, A] =
  ...
  final def futureFork[R: _future, A](
    a: => A,
    ec: ExecutionContext,
    timeout: Option[FiniteDuration] = None
  ): Eff[R, A] =
...
trait FutureInterpretation extends FutureTypes {
  ...
  def runAsyncOn[R, A](executorServices: ExecutorServices)(e: Eff[R, A])(implicit
  ...
  def runAsync[R, A](
    e: Eff[R, A]
  )(implicit scheduler: Scheduler, exc: ExecutionContext, m: Member.Aux[TimedFuture, R, NoFx]): Future[A] =
    Eff
      .detachA(Eff.effInto[R, Fx1[TimedFuture], A](e))(TimedFuture.MonadTimedFuture, TimedFuture.ApplicativeTimedFuture)
      .runNow(scheduler, exc)

  def runSequentialOn[R, A](executorServices: ExecutorServices)(e: Eff[R, A])(implicit
    m: Member.Aux[TimedFuture, R, NoFx]
  ): Future[A] =
