...
trait SafeInterpretation extends SafeCreation { outer =>
   ...
   */
  def runSafe[R, U, A](effect: Eff[R, A])(implicit
    m: Member.Aux[Safe, R, U]
  ): Eff[U, (ThrowableEither[A], List[Throwable])] =
  ...
  def safeInterpreter[R, A](last: Option[(Eff[R, Unit], Safe /= R)]): Interpreter[Safe, R, A, Out[A]] =
    new Interpreter[Safe, R, A, Out[A]] {
      ...
      def onApplicativeEffect[X, T[_]: Traverse](
        xs: T[Safe[X]],
        continuation: Continuation[R, T[X], Out[A]]
      ): Eff[R, Out[A]] = {
  ...
  def thenFinally[R, A](effect: Eff[R, A], last: Eff[R, Unit])(implicit m: Safe /= R): Eff[R, A] =
    intercept[R, Safe, A, Out[A]](Eff.whenStopped(effect, Last.eff(last)))(
      safeInterpreter[R, A](Some((last, m)))
    ).flatMap {
  ...
  def bracket[R, A, B, C](
    acquire: Eff[R, A]
  )(use: A => Eff[R, B])(release: A => Eff[R, C])(implicit m: Safe /= R): Eff[R, B] =
    for {
      a <- acquire
   ...
   */
  def catchThrowable[R, A, B](action: Eff[R, A], pureValue: A => B, onThrowable: Throwable => Eff[R, B])(implicit
    m: Safe /= R
  ): Eff[R, B] =
   ...
   */
  def recoverThrowable[R, A, B](
    action: Eff[R, A],
    pureValue: A => B,
    onThrowable: PartialFunction[Throwable, Eff[R, B]]
  )(implicit
    m: Safe /= R
  ): Eff[R, B] =
    attemptSafe(action).flatMap {
      case (Left(t), ls) if onThrowable.isDefinedAt(t) =>
        onThrowable(t).flatMap(b => ls.traverse(f => finalizerException(f)).as(b))
   ...
   * The final value type is the same as the original type
   */
  def whenThrowable[R, A](action: Eff[R, A], onThrowable: PartialFunction[Throwable, Eff[R, A]])(implicit
    m: Safe /= R
  ): Eff[R, A] =
