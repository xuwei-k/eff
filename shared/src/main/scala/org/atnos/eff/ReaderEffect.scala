...
trait ReaderInterpretation {
   ...
   */
  def runKleisli[R, U, S, A, F[_]](
    env: S
  )(e: Eff[R, A])(implicit mx: Member.Aux[Kleisli[F, S, *], R, U], m: F |= U): Eff[U, A] =
   ...
   */
  def translateReader[R, U, S, B, A](e: Eff[R, A], getter: B => S)(implicit
    sr: Member.Aux[Reader[S, *], R, U],
    br: Reader[B, *] |= U
  ): Eff[U, A] =
    translate(e) {
  ...
  def zoomReader[R1, R2, U, S, T, A](
    ...
    f: T => S
  )(implicit readerS: Member.Aux[Reader[S, *], R1, U], readerT: Member.Aux[Reader[T, *], R2, U]): Eff[R2, A] =
    transform(
      e,
