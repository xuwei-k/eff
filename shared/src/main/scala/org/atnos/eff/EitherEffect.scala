...
trait EitherInterpretation {
  ...
  def runEitherCombine[R, U, E, A](
    effect: Eff[R, A]
  )(implicit m: Member.Aux[Either[E, *], R, U], s: Semigroup[E]): Eff[U, E Either A] =
  ...
  def runEitherCatchLeft[R, U, E, A](r: Eff[R, A])(handle: E => Eff[U, A])(implicit
    m: Member.Aux[Either[E, *], R, U]
  ): Eff[U, A] =
  ...
  def catchLeftCombine[R, E, A](effect: Eff[R, A])(
    handle: E => Eff[R, A]
  )(implicit member: Either[E, *] /= R, s: Semigroup[E]): Eff[R, A] =
    catchLeftEither[R, E, A](effect)(handle)(EitherApplicative[E])

  private def catchLeftEither[R, E, A](effect: Eff[R, A])(handle: E => Eff[R, A])(ap: Applicative[Either[E, *]])(
    implicit member: Either[E, *] /= R
  ): Eff[R, A] =
    Interpret.intercept(effect)(Interpreter.fromRecurser(eitherRecurser[R, E, A, A](a => a, handle)(ap)))

  private def eitherRecurser[R, E, A, B](pureValue: A => B, handle: E => Eff[R, B])(
    ap: Applicative[Either[E, *]]
  ): Recurser[Either[E, *], R, A, B] =
...
trait EitherImplicits {
  ...
  implicit final def errorTranslate[R, E1, E2](implicit
    m: MemberIn[Either[E1, *], R],
    map: E2 => E1
  ): MemberIn[Either[E2, *], R] =
    m.transform(errorTranslateNat(map))

  final def errorTranslateNat[E1, E2](map: E2 => E1): Either[E2, *] ~> Either[E1, *] =
    new (Either[E2, *] ~> Either[E1, *]) {
