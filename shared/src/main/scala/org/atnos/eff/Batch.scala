        ...
        collected.effects zip collected.indices match {
          ...
          case e +: rest =>
            ...
            result.effects match {
              ...
              case e1 +: rest1 =>
                ImpureAp(Unions(m.inject[Any](e1.asInstanceOf[T[Any]]), rest1.map(r => m.inject(r.asInstanceOf[T[Any]])) ++ collected.otherEffects),
...
private case class Single[T[_]](tx: T[_], keys: Vector[Int]) extends Batched[T] {
  ...
  def append(ty: T[_], key: Int): Batched[T] =
    Composed(Vector(Batched.single[T, Any]((tx.asInstanceOf[T[Any]], key))), this)
