      ...
      case ImpureAp(unions, continuation, last) =>
        ...
        collected.effects zip collected.indices match {
          ...
          case e +: rest =>
            ...
            val result: Batched[T] = rest.foldLeft(Batched.single[T, Any](e)) { case (batched, (effect, i)) =>
              batchable.batch(batched.batchedEffect.asInstanceOf[T[Any]], effect) match {
            ...
            result.effects match {
              ...
              case e1 +: rest1 =>
                ImpureAp(Unions(m.inject(e1.asInstanceOf[T[R]]), rest1.map(r => m.inject(r.asInstanceOf[T[Any]])) ++ collected.otherEffects),
...
private case class Composed[T[_]](unbatched: Vector[Batched[T]], batched: Single[T]) extends Batched[T] {
  ...
  def append(ty: T[_], key: Int): Composed[T] =
    copy(unbatched  = unbatched :+ Batched.single((ty.asInstanceOf[T[Any]], key)))
