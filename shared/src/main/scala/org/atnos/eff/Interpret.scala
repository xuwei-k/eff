...
trait Interpret {
  ...
  def runInterpreter[R, U, T[_], A, B](
    e: Eff[R, A]
  )(interpreter: Interpreter[T, U, A, B])(implicit m: Member.Aux[T, R, U]): Eff[U, B] = {
    ...
    def interpretLastEff(last: Eff[R, Unit]): Eff[U, Unit] =
      last match {
        ...
        case Impure(u: Union[_, _], c, last1) =>
          m.project(u) match {
            case Right(tu) =>
              interpreter.onLastEffect(
                tu,
                Continuation.lift((x: u.X) => interpretLastEff(c(x).addLast(last1)), interpretLast(c.onNone))
              )
            case Left(other) =>
              Impure(
                other,
                Continuation.lift((x: u.X) => interpretLastEff(c(x)), interpretLast(c.onNone)),
                interpretLast(last1)
              )
          }
    ...
    e match {
      ...
      case ap @ ImpureAp(unions, continuation, last) =>
        ...
        if (collected.effects.isEmpty)
          ...
          interpreter
            .onApplicativeEffect(collected.effects, collected.continuation(interpretContinuation(continuation)))
   ...
   */
  def recurse[R, U, T[_], A, B](e: Eff[R, A])(recurser: Recurser[T, U, A, B])(implicit
    m: Member.Aux[T, R, U]
  ): Eff[U, B] =
   ...
   */
  def translateInto[R, T[_], U, A](
    effect: Eff[R, A]
  )(tr: Translate[T, U])(implicit t: T /= R, into: IntoPoly[R, U]): Eff[U, A] = {
  ...
  def interpretUnsafe[R, U, T[_], A](effect: Eff[R, A])(sideEffect: SideEffect[T])(implicit
    m: Member.Aux[T, R, U]
  ): Eff[U, A] =
  ...
  def interceptNatM[R, M[_], F[_], A](effect: Eff[R, A], nat: M ~> (M `of` F)#l)(implicit
    ...
    intercept[R, M, A, F[A]](effect)(new Interpreter[M, R, A, F[A]] {
      ...
      def onEffect[X](mx: M[X], continuation: Continuation[R, X, F[A]]): Eff[R, F[A]] =
        Impure(
          m.inject(nat(mx)),
          Continuation.lift((fx: F[X]) => Eff.flatTraverseA(fx)(continuation), continuation.onNone)
        )
      ...
      def onLastEffect[X](mx: M[X], continuation: Continuation[R, X, Unit]): Eff[R, Unit] =
        Impure(
          m.inject(nat(mx)),
          Continuation
            .lift((fx: F[X]) => Eff.flatTraverseA(fx)(x => continuation(x).map(FM.pure)).void, continuation.onNone)
        )

      def onApplicativeEffect[X, T[_]: Traverse](
        xs: T[M[X]],
        continuation: Continuation[R, T[X], F[A]]
      ): Eff[R, F[A]] = {
        val xss = xs.toList.map(mx => nat(mx)).toVector.map(m.inject)
        ImpureAp(
          Unions(xss.head, xss.tail.asInstanceOf[Vector[Union[R, Any]]]),
          Continuation.lift(
            (tfx: Vector[Any]) => FT.map(tfx.asInstanceOf[T[F[X]]].sequence)(continuation).sequence.map(_.flatten),
            continuation.onNone
          )
        )
      }
   ...
   */
  def augment[R, T[_], O[_], A](
    eff: Eff[R, A]
  )(w: Augment[T, O])(implicit memberT: MemberInOut[T, R], memberO: MemberIn[O, R]): Eff[R, A] = {
  ...
  def write[R, T[_], O, A](
    eff: Eff[R, A]
  )(w: Write[T, O])(implicit memberT: MemberInOut[T, R], memberW: MemberIn[Writer[O, *], R]): Eff[R, A] = {
  ...
  def trace[R, T[_], A](
    eff: Eff[R, A]
  )(implicit memberT: MemberInOut[T, R], memberW: MemberInOut[Writer[T[_], *], R]): Eff[R, A] =
