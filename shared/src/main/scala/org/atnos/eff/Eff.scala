      ...
      fa match {
        case Pure(a, last) =>
          ff match {
            ...
            case Impure(NoEffect(f), c, last1)    => Impure(NoEffect[AnyRef, Any](f), c.append(f1 => pure(f1(a))).asInstanceOf, c.onNone).addLast(last1 *> last)
            case Impure(u: Union[_, _], c, last1) => ImpureAp(Unions(u, Vector.empty), c.dimapEff((_:Vector[Any]).head.asInstanceOf)(_.map(_(a))), last1 *> last)
        ...
        case Impure(u: Union[_, _], c, last) =>
          ff match {
            case Pure(f, last1)                     => ImpureAp(Unions(u, Vector.empty), c.contramap((_:Vector[Any]).head.asInstanceOf).map(f), last1 *> last)
            case Impure(NoEffect(f), c1, last1)     => Impure(u, c.append(x => c1(f).map(_(x)))).addLast(last1 *> last)
            case Impure(u1: Union[_, _], c1, last1) => ImpureAp(Unions(u, Vector(u1.asInstanceOf)),  Continuation.lift(ls => ap(c1(ls(1).asInstanceOf))(c(ls.head.asInstanceOf)), c.onNone), last1 *> last)
            case ImpureAp(u1, c1, last1)            => ImpureAp(Unions(u, u1.unions), Continuation.lift(ls => ap(c1(ls.drop(1)))(c(ls.head.asInstanceOf)), c.onNone), last1 *> last)
          }
        ...
        case ImpureAp(unions, c, last) =>
          ff match {
            ...
            case Impure(u: Union[_, _], c1, last1) => ImpureAp(Unions(unions.first, unions.rest :+ u.asInstanceOf), Continuation.lift(ls => ap(c1(ls.last.asInstanceOf))(c(ls.dropRight(1))), c.onNone), last1 *> last)
