      ...
      fa match {
        case Pure(a, last) =>
          ff match {
            ...
            case Impure(NoEffect(f), c, last1)    => Impure[AnyRef, Any, B](NoEffect[AnyRef, Any](f), c.append(f1 => pure(f1(a))).asInstanceOf[Continuation[AnyRef, Any, B]], c.onNone).addLast(last1 *> last)
