...
"""
  ...
  def catchThrowable2 = prop { (n: Int) =>
    val program =
      (protect[S, Int](action(n)) `finally` protect[S, Unit](throw finalBoom)).catchThrowable(identity, _ => pure(1))
  ...
  def catchThrowable3 = prop { (n: Int) =>
    val program =
      protect[S, Int](action(n)).catchThrowable(identity, _ => pure(1)) `finally` protect[S, Unit](throw finalBoom)
