...
final case class TwitterTimedFuture[A](
  callback: (FuturePool, Scheduler) => Future[A],
  ...
  def runNow(pool: FuturePool, scheduler: Scheduler): Future[A] =
    timeout.fold(callback(pool, scheduler)) { t =>
      ...
      callback(pool, scheduler).onFailure { e => cancelTimeout(); val _ = promise.updateIfEmpty(Throw(e)) }.onSuccess {
        a =>
          cancelTimeout(); val _ = promise.updateIfEmpty(Return(a))
      }
      promise
...
object TwitterTimedFuture {
  ...
  implicit final val MonadTwitterTimedFuture: MonadError[TwitterTimedFuture, Throwable] =
    new MonadError[TwitterTimedFuture, Throwable] {
      def pure[A](x: A): TwitterTimedFuture[A] =
        TwitterTimedFuture((_, _) => Future.value(x))
...
trait TwitterFutureCreation extends TwitterFutureTypes {
  ...
  final def fromFutureWithExecutors[R: _future, A](
    c: (FuturePool, Scheduler) => Future[A],
    timeout: Option[FiniteDuration] = None
  ): Eff[R, A] =
  ...
  def waitFor[R: _future](duration: FiniteDuration): Eff[R, Unit] =
    Eff.send(
      TwitterTimedFuture((_, scheduler) =>
...
trait TwitterFutureInterpretation extends TwitterFutureTypes {
  ...
  def runAsync[R, A](
    e: Eff[R, A]
  )(implicit pool: FuturePool, scheduler: Scheduler, m: Member.Aux[TwitterTimedFuture, R, NoFx]): Future[A] =
    Eff
      .detachA(e)(TwitterTimedFuture.MonadTwitterTimedFuture, TwitterTimedFuture.ApplicativeTwitterTimedFuture, m)
      .runNow(pool, scheduler)

  def runSequential[R, A](
    e: Eff[R, A]
  )(implicit pool: FuturePool, scheduler: Scheduler, m: Member.Aux[TwitterTimedFuture, R, NoFx]): Future[A] =
   ...
   */
  final def futureMemo[R, A](key: AnyRef, cache: Cache, e: Eff[R, A])(implicit
    future: TwitterTimedFuture /= R
  ): Eff[R, A] =
    futureAttempt(Eff.memoizeEffect(e, cache, key)).flatMap {
      case Left(t) =>
        Eff.send(TwitterTimedFuture.twitterFutureSequenceCached.reset(cache, key)) >> TwitterFutureEffect.futureFail(t)
   ...
   * if this method is called with the same key the previous value will be returned
   */
  final def futureMemoized[R, A](key: AnyRef, e: Eff[R, A])(implicit
    future: TwitterTimedFuture /= R,
    m: Memoized |= R
  ): Eff[R, A] =
    MemoEffect.getCache[R].flatMap(cache => futureMemo(key, cache, e))

  def runFutureMemo[R, U, A](
    cache: Cache
  )(effect: Eff[R, A])(implicit m: Member.Aux[Memoized, R, U], task: TwitterTimedFuture |= U): Eff[U, A] = {
