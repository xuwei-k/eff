...
class TwitterFutureEffectSpec(implicit ee: ExecutionEnv)
...
"""
  ...
  implicit val scheduler: org.atnos.eff.concurrent.Scheduler =
    ExecutorServices.schedulerFromScheduledExecutorService(ee.ses)
  ...
  def e7 = {
    Await.result(futureFromEither(Left[Throwable, Int](boomException)).twitterFutureAttempt.runSequential) must beLeft(
      boomException
    )
  }
  ...
  def e8 = {
    Await.result(
      futureDelay({ sleepFor(10000.millis); 1 }, timeout = Some(50.millis)).twitterFutureAttempt.runSequential
    ) must beLeft
  }
  ...
  def e11 = {
    ...
    val cache = ConcurrentHashMapCache()
    def makeRequest =
      futureMemo("only once", cache, futureDelay({ invocationsNumber += 1; 1 }, timeout = Option(10000.millis)))
    ...
    def makeRequest =
      if (firstTime)
        futureMemo(
          "only once",
          cache,
          futureDelay { firstTime = false; throw new Exception("") } >> futureDelay { invocationsNumber += 1; 1 }
        )
      else
