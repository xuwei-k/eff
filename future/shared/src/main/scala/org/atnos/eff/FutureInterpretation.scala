...
trait FutureInterpretation extends FutureTypes {
  ...
  final def futureAttempt[R, A](e: Eff[R, A])(using future: TimedFuture /= R): Eff[R, Throwable Either A] =
  ...
  /**
    ...
    */
  final def futureMemo[R, A](key: AnyRef, cache: Cache, e: Eff[R, A])(using future: TimedFuture /= R): Eff[R, A] =
  ...
  /**
    ...
    * if this method is called with the same key the previous value will be returned
    */
  final def futureMemoized[R, A](key: AnyRef, e: Eff[R, A])(using future: TimedFuture /= R, m: Memoized |= R): Eff[R, A] =
