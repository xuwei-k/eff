...
trait validate {
  ...
  def runValidationNel[R, U, E, A](r: Eff[R, A])(using m: Member.Aux[Validate[E, *], R, U]): Eff[U, ValidationNel[E, A]] =
  ...
  def runNelDisjunction[R, U, E, A](r: Eff[R, A])(using m: Member.Aux[Validate[E, *], R, U]): Eff[U, NonEmptyList[E] \/ A] =
    org.atnos.eff.ValidateEffect.runNel(r).map(_.fold(ls => \/.left(NonEmptyList.fromSeq(ls.head, ls.tail)), \/.right))

  def runMapDisjunction[R, U, E, L: Semigroup, A](r: Eff[R, A])(map: E => L)(using m: Member.Aux[Validate[E, *], R, U]): Eff[U, L \/ A] =
