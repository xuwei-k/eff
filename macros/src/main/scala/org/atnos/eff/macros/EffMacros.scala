...
class EffMacros(val c: blackbox.Context) {
  ...
  // remove () if no args
  def methodCallFmt(method: c.universe.Tree, args: Seq[Seq[TermName]]) =
  ...
  def impl(annottees: c.Expr[Any]*): c.Expr[Any] = {
    ...
    trees.headOption match {
      case Some(
            q"$_ trait ${tpname: TypeName}[..$_] extends { ..$earlydefns } with ..$parents { $self => ..$stats }"
          ) =>
        ...
        stats.foreach {
          case x: ValOrDefDef if x.mods.hasFlag(Flag.PRIVATE | Flag.PROTECTED) =>
            c.abort(x.pos, "try using access modifier: package-private")
          case v @ ValDef(_, _, rt: TypeTree, _) =>
            c.abort(v.pos, s"Define the return type for:") // requires explicit return type
          case d @ DefDef(_, _, _, _, rt: TypeTree, _) =>
            c.abort(d.pos, s"Define the return type for:") // requires explicit return type
          case v @ ValDef(mods, _, rt, _) if mods.hasFlag(Flag.MUTABLE) =>
            c.abort(v.pos, s"var is not allow in @eff trait $tpname")
          case v @ ValDef(_, _, rt, EmptyTree) =>
            if (!isReturnTypeOfTypeAlias(rt))
              c.abort(
                v.pos,
                s"Abstract val needs to have return type ${typeAlias.name}[...], otherwise, make it non-abstract."
              )
          case d @ DefDef(_, _, _, _, rt, EmptyTree) =>
            if (!isReturnTypeOfTypeAlias(rt))
              c.abort(
                d.pos,
                s"Abstract def needs to have return type ${typeAlias.name}[...], otherwise, make it non-abstract."
              )
          case _ => // no issue
        }

        val absValsDefsOps: Seq[ValOrDefDef] = stats.collect {
          case m @ DefDef(_, _, _, _, ExpectedReturnType(_), EmptyTree) =>
            m
        }
          ...
          absValsDefsOps.collect {
            case q"..$_ def $tname[..$tparams](...$paramss): ${AppliedTypeTree(_, returnType)} = $expr" =>
              ...
              val implicitParams = implicits.collect {
                case ValDef(_, _, AppliedTypeTree(Ident(name), Seq(Ident(stackTypeName))), _)
              ...
              q"case class ${TypeName(tname.toString.capitalize)}[..$nonStackTypeParams](..${nonStackParams(
                  fixedParams
                ).flatten}) extends ${sealedTrait.name}[..$nonStackReturnTypes]"
          }
        ...
        val sideEffectTrait = {
          ...
          q"""
              ...
              case DefDef(_, name, _, paramss, rt, _) =>
                val binds = nonStackParams(paramss).flatMap(_.collect { case t: ValDef =>
                  Bind(t.name, Ident(termNames.WILDCARD))
                })
        ...
        val translatorFactoryTraits = (1 until 10).map { numOutEffects =>
          ...
          val effectImplicitParams = (1 to numOutEffects)
            .map(i =>
              ValDef(Modifiers(Flag.IMPLICIT), replacementParamName(i), tq"${TypeName(s"R$i")} MemberIn U", EmptyTree)
            )
            .toList
          val effectImplicitArgs = (1 to numOutEffects).map(replacementParamName)
          val typeU =
            TypeDef(Modifiers(Flag.PARAM), TypeName("U"), List(), TypeBoundsTree(tq"scala.Nothing", tq"scala.Any"))
          ...
          val methodsToBeImpl: Seq[DefDef] = absValsDefsOps.map {
            case q"..$mods def $name[..$tparams](...$paramss): Eff[$_, $returnType]" =>
              DefDef(
                mods,
                name,
                tparams.dropRight(1) :+ typeU,
                addImplicits(effectImplicitParams, nonStackParams(paramss)),
                tq"Eff[U, $returnType]",
                EmptyTree
              )
          }
          ...
          val cases = absValsDefsOps.map {
            case q"..$mods def $name[..$tparams](...$paramss): Eff[$_, $returnType]" =>
              ...
              val args = addImplicits(effectImplicitParams, nonStackParams(paramss)).map(_.collect { case t: ValDef =>
                t.name
              })
              val rhs = q"$name(...$args)"
              cq"${adt(name)}(..$binds) => $rhs.asInstanceOf[Eff[U, X]]"
          ...
          q"""
              trait $factoryTrait[..${(1 to numOutEffects).map(i =>
              TypeDef(
                Modifiers(Flag.PARAM),
                TypeName(s"R$i"),
                List(
                  TypeDef(Modifiers(Flag.PARAM), typeNames.WILDCARD, List(), TypeBoundsTree(TypeTree(), TypeTree()))
                ),
