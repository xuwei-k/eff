...
trait ValidateInterpretation extends ValidateCreation {
  ...
  def runNel[R, U, E, A](r: Eff[R, A])(using m: Member.Aux[Validate[E, *], R, U]): Eff[U, Either[NonEmptyList[E], A]] =
    runMap[R, U, E, NonEmptyList[E], A](r)((e: E) => NonEmptyList.of(e))

  /** run the validate effect, yielding a list of failures Either A */
  def runMap[R, U, E, L: Semigroup, A](effect: Eff[R, A])(map: E => L)(using m: Member.Aux[Validate[E, *], R, U]): Eff[U, Either[L, A]] =
    runMapGen(effect)(map) { (a, l) => l.map(_ => a) }

  /** run the validate effect, yielding a non-empty list of failures or A or both */
  def runIorNel[R, U, E, A](r: Eff[R, A])(using m: Member.Aux[Validate[E, *], R, U]): Eff[U, IorNel[E, A]] =
  ...
  def runIorMap[R, U, E, L: Semigroup, A](effect: Eff[R, A])(map: E => L)(using m: Member.Aux[Validate[E, *], R, U]): Eff[U, Ior[L, A]] =
  ...
  private def runMapGen[R, U, E, L: Semigroup, A, SomeOr[_, _]](
    effect: Eff[R, A]
  )(map: E => L)(pure: (A, Either[L, Option[L]]) => SomeOr[L, A])(using m: Member.Aux[Validate[E, *], R, U]): Eff[U, SomeOr[L, A]] =
