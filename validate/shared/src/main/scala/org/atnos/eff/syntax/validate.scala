...
trait validate {
  ...
  given validateExtension: AnyRef with {
    extension [R, A](e: Eff[R, A]) {

      def runNel[E](using m: Member[Validate[E, *], R]): Eff[m.Out, Either[NonEmptyList[E], A]] =
        ValidateInterpretation.runNel(e)(using m.aux)

      def runMap[E, L: Semigroup](map: E => L)(using m: Member[Validate[E, *], R]): Eff[m.Out, Either[L, A]] =
      ...
      def runIorMap[E, L: Semigroup](map: E => L)(using m: Member[Validate[E, *], R]): Eff[m.Out, Ior[L, A]] =
