...
To do this, we will use an interpreter transforming our `KVStore` effects using a simple mutable map: ${definition[
      AdtInterpreterSnippet
    ]}
...
Then you simply require your program to have `MemberIn` instances for those effects:${definition[
      UserInteractionProgramSnippet
    ]}
