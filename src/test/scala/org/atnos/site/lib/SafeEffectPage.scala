...
object SafeEffectPage extends UserGuidePage {
  def is = "Safe".title ^ s2"""
...
// our stack of effects, with safe evaluation
      ...
      def useResource(ok: Boolean) =
        (r: Resource) => protect[S, Int](if (ok) r.values.sum else throw new Exception("boo"))
...
// this program tries to set sumDone to true when the computation is done
      def program(ok: Boolean, finalizeOk: Boolean): (Throwable Either Int, List[Throwable]) =
        (protect[S, Int](if (ok) (1 to 10).sum else throw new Exception("boo")) `finally` setDone(
          finalizeOk
        )).runSafe.run
...
"""
  ...
  def showResult1(
    message: String,
    result: (Throwable Either Int, List[Throwable]),
    ...
    result match {
      case (r, ls) =>
        s"""|
            |$message: ${r.leftMap(_.getMessage)}, finalizers exceptions: ${if (ls.isEmpty) "no exceptions"
  ...
  def showResult2(message: String, result: (Throwable Either Int, List[Throwable])) =
    result match {
      case (r, ls) =>
        s"""|
          |$message: ${r.leftMap(_.getMessage)}, finalizers exceptions: ${if (ls.isEmpty) "no exceptions"
          else ls.map(_.getMessage).toString}""".stripMargin
    }
