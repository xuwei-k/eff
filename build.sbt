...
lazy val twitterUtilVersion = "21.8.0"
...
lazy val scalaz = crossProject(JSPlatform, JVMPlatform, NativePlatform).in(file("scalaz"))
  ...
  .settings(
    libraryDependencies += "org.scalaz" %%% "scalaz-core" % "7.3.4" cross CrossVersion.for3Use2_13,
  )
  .settings(effSettings)
  .jvmSettings(commonJvmSettings)
  .jsSettings(commonJsSettings)
...
lazy val buildSettings = Seq(
  ...
  crossScalaVersions := Seq(Scala212, "2.13.6", "3.0.2-RC1")
)
